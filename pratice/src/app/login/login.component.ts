...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.loginForm = this.fb.group({
      ...
      password: ['', [Validators.required, Validators.minLength(8),Validators.pattern('(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@$!%*?&_#])[A-Za-z\d$@$!%*?&].{7,}')]],
    })
    ...
    if (this.loginForm.valid) {
      ...
      this.service.LoginPost(data).subscribe(res => {
        ...
        if (this.userData.Status == 200) {
          ...
          sessionStorage.setItem('role', roll);
          sessionStorage.setItem('isLogin', 'true');
          this.snackBar.open(this.userData.Info, '', { verticalPosition: 'top', duration: 2000, panelClass: ['green-snackbar'] });
          sessionStorage.setItem('userData', JSON.stringify(this.userData));
          this.router.navigate(["user/home"]);
        }  
      }
      , error => {
          console.log(error);
          if (error.error.Status == 401||404||500||403) {
